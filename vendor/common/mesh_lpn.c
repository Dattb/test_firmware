...
{
	return mesh_lpn_par.link_ok; //RD_EDIT : friend link ok
}
...
void mesh_friend_ship_proc_LPN(u8 *bear)
{
    ...
    if(0 == fri_ship_proc_lpn.status){
        ...
        switch(fri_ship_proc_lpn.status){   // Be true only during establish friend ship.
            case FRI_ST_REQUEST:
            	//RD_EDIT: friend request off
//            	if(run_onetime){
//
//            		run_onetime =0;
//
//        			if(pmParam.is_pad_wakeup) friend_request_cnt = 3;
//            		if(	friend_request_cnt == 0 || friend_request_cnt > 3){
//
//            			if(friend_request_cnt > 3) friend_request_cnt = 1;
	    ...
	    if(!is_provision_success()){
	        if(!is_provision_working()){   // not being provision
    	        if(clock_time_exceed(0, provision_time*1000)){
    	            light_onoff_all(0);
    	            //RD_EDIT: wakeup khi ngoai mang
                    cpu_sleep_wakeup(DEEPSLEEP_MODE_RET_SRAM_LOW32K, PM_WAKEUP_PAD|DEEPWAKEUP_SRC_TIMER, clock_time() + 200*CLOCK_16M_SYS_TIMER_CLK_1S);
                    while(1);   // wait reboot
    	        }
	        }
	    }else{
