...
#include "vendor/Rang_Dong/RD_BL55072A_LCD.h"
#include "vendor/Rang_Dong/RD_SHTC3_Sensor.h"
//#include "../Rang_Dong/RD_Light_Sensor.h"


extern u16 provision_time;
extern u8 factory_test_CLD_flag;
u16 wave_cnt=0;
u8  lCD_test_mode = 0;
u8 Pin_cnt = 0;
u8  wave_level=0;
u8 factory_test_wave_cnt = 0;
...
//	gpio_write(GPIO_PB4,1);                 //LED On
	...
	#else
	//access_cmd_onoff(0xffff, 0, (test_onoff++) & 1, CMD_NO_ACK, 0); // RD-EDIT: nho tat dong nay o cac cam bien khac
	...
	if(fri_request_send/* || clock_time_exceed(A_req_tick, 2000*1000)*/){
		...
		//if((!is_in_mesh_friend_st_lpn()) && (!fri_ship_proc_lpn.status)){
			if(is_provision_success()){
				...
				//RD_EDIT: ham show wave khi vao mang
...
u8 reset_cnt=0;
u8 factory_test_mode = 0;
void main_loop ()
{
	wave_cnt++;
		//RD_EDIT: show provision bang wave
	 if(get_provision_state()==STATE_DEV_PROVING){

		if(wave_cnt>100){
			wave_cnt=0;
			wave_level++;
			if(wave_level>3)wave_level=0;
			//RD_LCD_Set_data(RD_Read_Raw_Temp_Hum_data().temperature,RD_Read_Raw_Temp_Hum_data().humidity,100);
			LCD_Show_provision_state();
			RD_LCD_printf();
		}
	}
	else if(get_provision_state() == STATE_DEV_UNPROV){
			if(factory_test_CLD_flag){
				if(wave_cnt>400){

					wave_cnt=0;
					lCD_test_mode++;
					Pin_cnt++;
					factory_test_wave_cnt++;
					if(lCD_test_mode>9){
						factory_test_mode ++;
						lCD_test_mode =0;

					}
					if(Pin_cnt>5) Pin_cnt = 0;
					if(factory_test_wave_cnt>4) factory_test_wave_cnt =0;
					if(factory_test_mode<3) {
						factory_test_LCD(factory_test_CLD_flag,lCD_test_mode,Pin_cnt,factory_test_wave_cnt);
						RD_LCD_printf();
					}
					else {
						factory_test_LCD(factory_test_CLD_flag,8,5,4);
						RD_LCD_printf();
					}
...
void user_init()
{
	...
	//RD_EDIT: cho nay de hien thi LCD khi o ngoai mang ma khong bi treo

	if(get_provision_state()==STATE_DEV_UNPROV){
		if(pmParam.wakeup_src!= PM_WAKEUP_TIMER ){
			RD_LCD_Set_data(RD_Read_Raw_Temp_Hum_data().temperature,RD_Read_Raw_Temp_Hum_data().humidity,(u16)(RD_power_read()));
			RD_LCD_printf();
		}
		lpn_key_factory_reset_check();
	}
